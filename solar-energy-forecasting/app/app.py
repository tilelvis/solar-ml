...
def load_data():
    ...
    return raw_df, feature_columns   # 🔹 Always return 2 values
...
def prepare_features(forecast_time, temp, irradiance, wind_speed, historical_data):
    ...
    Returns both the feature DataFrame and a flag if NaNs were filled.
    """
    nan_filled = False  # track if we had to fallback
    ...
    for window in [3, 6, 12, 24]:
        combined_temp = pd.concat([hist_subset['temperature'].tail(window-1),
                                   pd.Series(temp, index=[forecast_time])])
        combined_irrad = pd.concat([hist_subset['irradiance'].tail(window-1),
                                    pd.Series(irradiance, index=[forecast_time])])
    ...
    # 🔹 Final step: replace any remaining NaNs
    if pred_df.isna().any().any():
        nan_filled = True
        pred_df = pred_df.fillna(0)

    return pred_df, nan_filled
...
if model is None or raw_df is None:
    ...
    with col1:
        ...
        if st.button("Get Forecast", type="primary"):
            # Prepare features
            features, nan_filled = prepare_features(
                forecast_datetime,
                temp_input,
                irrad_input,
                wind_input,
            ...
            if nan_filled:
                st.warning("⚠️ Some lag/rolling features were missing and were filled with 0. "
                           "Prediction may be less accurate due to limited historical data.")
